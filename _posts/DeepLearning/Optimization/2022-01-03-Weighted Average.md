...
전체 날씨 개수가 무한하다고 할 때 , 단순히 반영비율이 0.7이 아닌 원하는 반영 비율을 구할 수 도 있습니다.
  ...
  Weighted avearge 가 최신 k개의 데이터의 Weighted avearge임을 보인다고 가정하겠습니다. 
...
만약 ,  gradient가 일정하다면 , 가속도(gradient) 와 마찰력((1-$$\beta$$)v)[[1]](#comments) 가 같아지는 순간 v는 더 증가하지 않게 됩니다. 이를 terminal velocity라 합니다. 
...
1 . 마찰력은 힘. 즉, F = ma. 가속도는 속도의 변화량이다. 따라서, ((1-$$\beta$$)v)이다.