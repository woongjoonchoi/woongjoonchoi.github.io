...
for k K
  for i  M
    for j N
      c[i][j] += A[i][k] *B[k][j]
```
하지만, 이렇게하면 B에서 access하는 data가 연속적이지 않으므로 data locality가 낮아지게 됩니다.
...
### Block Tiling
Matrix를 전체를 한번에 cache에 load하기는 어렵고 매번 element를  load하는 것은 비효율적입니다.
따라서, matrix의 일부를 cache에 load해서 계산을 하도록 합니다. 이렇게 matrix 일부를 load해서 계산하는것은 Matrix Multiplication을 Block Multiplication으로 해석한 것입니다.
![Block-Tiling](\assets\images\software_engineering\gpu_programming\Block-tiling.png)  
Output Matrix에서 Block을 설정하여 GPU의 core들이 각 block을 병렬적으로 계산하게 됩니다.
Block Multiplication은 상대적으로 작은 matrix들의 연산이기 때문에 이를 cache memory에 load할 수 있습니다. 따라서, GPU Programming에서는 shared memory에 명시적으로 load할 수 있는 syntax를 허용해줍니다.
따라서 ,psuedo code를 작성하면 아래와 같습니다.
```
...
Block Multiplication은 output blcok을 계산할 때 A의 block을 N//bn 번, B의 block을 M//bm번 load를 합니다.따라서, 각 element들이 A에서는 N//bn번 , B에서는 M//bm번  DRAM으로부터 shared memory로 load됩니다.
n
